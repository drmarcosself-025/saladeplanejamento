...
begin
  ...
  v_result := public.reserve_outbound_bubble(
    ...
    p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from public.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null),
  ...
  perform public.advance_bubble_to_sending(v_msgA, v_job, 'worker-A', v_turn, v_lead, v_rev1,
    ...
    'AI_REPLY');
  v_result := public.finalize_bubble_send(v_msgA, v_job, 'worker-A', v_turn, 'SENT', 'EVO-DED-1');
  if not (v_result->>'finalized')::boolean then
    raise exception 'PONTO1b: finalize do turno 1 deveria aplicar (%)', v_result;
  end if;
  if (select send_status from public.messages where id = v_msgA) <> 'SENT' then
    raise exception 'PONTO1c: bolha do turno 1 deveria estar SENT';
  end if;

  perform public.close_turn(v_job, 'worker-A', 'COMPLETED',
    array(select id from public.messages where lead_id = v_lead and direction = 'IN'), true, null, 4, 120);
  ...
  perform public.advance_bubble_to_sending(v_msgB, v_job, 'worker-A', v_turn, v_lead, v_rev2,
    (select coalesce(array_agg(id), array[]::uuid[]) from public.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null),
    'AI_REPLY');
  v_result := public.finalize_bubble_send(v_msgB, v_job, 'worker-A', v_turn, 'SENT', 'EVO-DED-2');
  if not (v_result->>'finalized')::boolean then
  ...
  perform public.close_turn(v_job, 'worker-A', 'COMPLETED',
    array(select id from public.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null), true, null, 4, 120);
  ...
  insert into public.jobs (lead_id, status, debounce_started_at, run_after) values (v_lead, 'PENDING', now(), now());
  update public.jobs set run_after = now() where lead_id = v_lead and status = 'PENDING';
  select job_id, turn_id into v_job, v_turn from public.claim_lead_jobs('worker-A', 5, 120);
  ...
  update public.jobs set status = 'DONE' where lead_id = v_lead and status = 'RUNNING';
  ...
  update public.jobs set status = 'DONE', locked_by = null, lease_expires_at = null where lead_id = v_lead and status = 'RUNNING';
  ...
  v_rev1 := (v_result->>'revision')::bigint;
  update public.jobs set run_after = now() where lead_id = v_lead and status = 'PENDING';
  select job_id, turn_id into v_job, v_turn from public.claim_lead_jobs('worker-B', 5, 120);
  ...
  if (select provider_message_id from public.messages where id = v_msg) <> 'EVO-UNICO-1' then
  ...
  v_result := public.reserve_outbound_bubble(
    p_job_id => v_job, p_worker_id => 'worker-A', p_lead_id => v_lead,
    p_input_revision => v_rev1, p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from public.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null),
    p_purpose => 'AI_REPLY', p_turn_id => v_turn, p_sequence => 1,
    p_text => 'POST em voo quando o lease morre', p_content_hash => 'hash-corrida', p_dedupe_seconds => 120);
  v_msg := (v_result->>'message_id')::uuid;

  perform public.advance_bubble_to_sending(v_msg, v_job, 'worker-A', v_turn, v_lead, v_rev1, (select coalesce(array_agg(id), array[]::uuid[]) from public.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null), 'AI_REPLY');
  ...
  if public.reconcile_stuck_sending_bubbles(45000) > 0
     and (select send_status from public.messages where id = v_msg) = 'UNKNOWN' then
  ...
  if public.reconcile_stuck_sending_bubbles(45000) < 1 then
  ...
  v_result := public.finalize_bubble_send(v_msg, v_job, 'worker-A', v_turn, 'SENT', 'EVO-TARDIO-1');
  ...
  if (select meta->'late_confirmation' from public.messages where id = v_msg) is null then
  ...
  if (select meta->'late_confirmation'->>'provider_message_id' from public.messages where id = v_msg) <> 'EVO-TARDIO-1' then
    raise exception 'PONTO3i: meta.late_confirmation deveria guardar o provider_message_id de verdade';
  end if;

  update public.jobs set status = 'FAILED', locked_by = null, lease_expires_at = null where id = v_job;
  update public.leads set needs_human = false, automation_status = 'ACTIVE' where id = v_lead;
  ...
  perform public.advance_bubble_to_sending(v_msg2, v_job, 'worker-A', v_turn, v_lead,
    (select conversation_revision from public.leads where id = v_lead), (select coalesce(array_agg(id), array[]::uuid[]) from public.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null), 'AI_REPLY');

  v_result := public.finalize_bubble_send(v_msg2, v_job, 'worker-A', v_turn, 'SENT', 'EVO-FELIZ-1');
  if not (v_result->>'finalized')::boolean then

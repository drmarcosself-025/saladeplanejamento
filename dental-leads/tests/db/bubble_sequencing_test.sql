...
begin
  ...
  v_result := dental_leads.reserve_outbound_bubble(
    ...
    p_input_revision => v_rev, p_batch_ids => (select array_agg(id) from dental_leads.messages where lead_id=v_lead and direction='IN'),
  ...
  if (select send_status from dental_leads.messages where id = v_msg) <> 'PENDING' then
    raise exception 'CASO 1b: bolha recém-reservada deveria estar PENDING';
  end if;
  if (select reserved_job_id from dental_leads.messages where id = v_msg) <> v_job
     or (select reserved_worker_id from dental_leads.messages where id = v_msg) <> 'worker-A' then
    raise exception 'CASO 1c: reserved_job_id/reserved_worker_id deveriam estar gravados (auditoria)';
  end if;

  v_result := dental_leads.advance_bubble_to_sending(
    p_message_id => v_msg, p_job_id => v_job, p_worker_id => 'worker-A', p_turn_id => v_turn,
    p_lead_id => v_lead, p_input_revision => v_rev,
    p_batch_ids => (select array_agg(id) from dental_leads.messages where lead_id=v_lead and direction='IN'),
    p_purpose => 'AI_REPLY');
  ...
  update dental_leads.messages set send_status = 'SENT', provider_message_id = 'EVO-ID-1' where id = v_msg;

  -- ---------------------------------------------------------------------
  -- 2. CANCELLED só a partir de PENDING: uma bolha já SENDING (ou SENT) NÃO
  --    pode ser cancelada por cancel_reserved_bubble.
  -- ---------------------------------------------------------------------
  if dental_leads.cancel_reserved_bubble(v_msg, v_job, 'worker-A', v_turn, 'tentativa_invalida') then
    raise exception 'CASO 2: bolha já SENT nunca pode ser cancelada (só PENDING vira CANCELLED)';
  end if;
  if (select send_status from dental_leads.messages where id = v_msg) <> 'SENT' then
    raise exception 'CASO 2: send_status não deveria ter mudado';
  end if;

  perform dental_leads.close_turn(v_job, 'worker-A', 'COMPLETED', array(select id from dental_leads.messages where lead_id=v_lead and direction='IN'), true, null, 4, 120);
  ...
  update dental_leads.jobs set status = 'DONE', outcome = 'SUPERSEDED', locked_by = null, lease_expires_at = null
   where lead_id = v_lead and status = 'PENDING';

  insert into dental_leads.jobs (lead_id, status, debounce_started_at, run_after)
  values (v_lead, 'PENDING', now(), now()) returning id into v_job;
  select job_id, turn_id into v_job, v_turn from dental_leads.claim_lead_jobs('worker-A', 5, 120);

  v_result := dental_leads.reserve_outbound_bubble(
    p_job_id => v_job, p_worker_id => 'worker-A', p_lead_id => v_lead,
    p_input_revision => v_rev, p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null),
    p_purpose => 'AI_REPLY', p_turn_id => v_turn, p_sequence => 1,
  ...
  begin
    perform dental_leads.reserve_outbound_bubble(
      p_job_id => v_job, p_worker_id => 'worker-A', p_lead_id => v_lead,
      p_input_revision => v_rev, p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null),
      p_purpose => 'AI_REPLY', p_turn_id => v_turn, p_sequence => 1,
  ...
  perform dental_leads.close_turn(v_job, 'worker-A', 'NO_REPLY', array[]::uuid[], false, null, 4, 120);
  ...
  v_result := dental_leads.advance_bubble_to_sending(
    p_message_id => v_msg, p_job_id => v_job, p_worker_id => 'worker-A', p_turn_id => v_turn,
    p_lead_id => v_lead, p_input_revision => v_rev, p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null), p_purpose => 'AI_REPLY');
  ...
  if (select send_status from dental_leads.messages where id = v_msg) <> 'CANCELLED' then
    raise exception 'CASO 4c: a bolha deveria ter sido cancelada automaticamente';
  end if;

  update dental_leads.jobs set status='DONE', outcome='FAILED', locked_by=null, lease_expires_at=null where id = v_job;
  ...
  declare
    v_rev_no_momento_da_bolha_1 bigint := (select conversation_revision from dental_leads.leads where id = v_lead);
  begin
    -- bolha 1: sai normalmente
    v_result := dental_leads.reserve_outbound_bubble(
      p_job_id => v_job, p_worker_id => 'worker-A', p_lead_id => v_lead,
      p_input_revision => v_rev_no_momento_da_bolha_1, p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null),
      p_purpose => 'AI_REPLY', p_turn_id => v_turn, p_sequence => 1,
      p_text => 'bolha 1 desta sequencia', p_content_hash => 'hash-entre-bolhas-1', p_dedupe_seconds => 120);
    v_msg := (v_result->>'message_id')::uuid;
    perform dental_leads.advance_bubble_to_sending(v_msg, v_job, 'worker-A', v_turn, v_lead,
      ...
      'AI_REPLY');
    update dental_leads.messages set send_status = 'SENT', provider_message_id = 'EVO-ID-ENTRE-1' where id = v_msg;
    ...
    v_result := dental_leads.advance_bubble_to_sending(
      ...
      p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null), p_purpose => 'AI_REPLY');
    if (v_result->>'advanced')::boolean then
    ...
    if (select send_status from dental_leads.messages where id = v_msg2) <> 'CANCELLED' then
  ...
  update dental_leads.jobs set status='DONE', outcome='PARTIAL_STALE', locked_by=null, lease_expires_at=null where id = v_job;
  ...
  v_msg := (v_result->>'message_id')::uuid;
  perform dental_leads.advance_bubble_to_sending(v_msg, v_job, 'worker-A', v_turn, v_lead,
    (select conversation_revision from dental_leads.leads where id = v_lead),
    (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null),
    'AI_REPLY');
  update dental_leads.messages set send_status = 'SENT', provider_message_id = 'EVO-ID-TAKEOVER-1' where id = v_msg;

  -- humano assume no meio da sequência
  update dental_leads.leads set automation_status = 'HUMAN_TAKEOVER' where id = v_lead;

  v_result := dental_leads.reserve_outbound_bubble(
    p_job_id => v_job, p_worker_id => 'worker-A', p_lead_id => v_lead,
    p_input_revision => (select conversation_revision from dental_leads.leads where id = v_lead),
    p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead and direction = 'IN' and consumed_at is null), p_purpose => 'AI_REPLY', p_turn_id => v_turn, p_sequence => 2,
  ...
  update dental_leads.leads set automation_status = 'ACTIVE' where id = v_lead;
  update dental_leads.jobs set status='DONE', outcome='HUMAN_TAKEOVER', locked_by=null, lease_expires_at=null where id = v_job;
  ...
  perform dental_leads.reconcile_stuck_sending_bubbles();
  if (select send_status from dental_leads.messages where id = v_msg) <> 'SENDING' then
  ...
  if dental_leads.reconcile_stuck_sending_bubbles() < 1 then
  ...
  update dental_leads.jobs set status='FAILED', outcome='FAILED', locked_by=null, lease_expires_at=null where id = v_job;
  ...
  update dental_leads.jobs set run_after = now() where lead_id = v_lead2 and status = 'PENDING';
  select job_id, turn_id into v_job2, v_turn2 from dental_leads.claim_lead_jobs('worker-B', 5, 120);

  v_result := dental_leads.reserve_outbound_bubble(
    p_job_id => v_job2, p_worker_id => 'worker-B', p_lead_id => v_lead2,
    p_input_revision => (select conversation_revision from dental_leads.leads where id = v_lead2),
    p_batch_ids => (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead2 and direction = 'IN' and consumed_at is null), p_purpose => 'AI_REPLY', p_turn_id => v_turn2, p_sequence => 1,
    p_text => 'evolution aceitou mas eu morri', p_content_hash => 'hash-crash-2xx', p_dedupe_seconds => 120);
  v_msg2 := (v_result->>'message_id')::uuid;
  perform dental_leads.advance_bubble_to_sending(v_msg2, v_job2, 'worker-B', v_turn2, v_lead2,
    (select conversation_revision from dental_leads.leads where id = v_lead2),
    (select coalesce(array_agg(id), array[]::uuid[]) from dental_leads.messages where lead_id = v_lead2 and direction = 'IN' and consumed_at is null),
    'AI_REPLY');
  ...
  update dental_leads.jobs set lease_expires_at = now() - interval '1 second' where id = v_job2;
  update dental_leads.messages set sending_at = now() - interval '1 hour' where id = v_msg2;
  perform dental_leads.reconcile_stuck_sending_bubbles();

  if (select send_status from dental_leads.messages where id = v_msg2) <> 'UNKNOWN' then
  ...
  if (select provider_message_id from dental_leads.messages where id = v_msg2) <> 'EVO-ID-TARDIO' then
    raise exception 'CASO 9c: o provider_message_id deveria ter sido carimbado retroativamente';
  end if;

  update dental_leads.leads set needs_human = false, automation_status = 'ACTIVE' where id = v_lead2;
  update dental_leads.jobs set status='FAILED', outcome='FAILED', locked_by=null, lease_expires_at=null where id = v_job2;
  ...
  if dental_leads.cancel_reserved_bubble(v_msg, v_job, 'worker-A', gen_random_uuid(), 'motivo') then
    raise exception 'CASO 10a: turn_id errado nunca pode cancelar (fencing)';
  end if;
  -- worker errado não cancela
  if dental_leads.cancel_reserved_bubble(v_msg, v_job, 'worker-intruso', v_turn, 'motivo') then
    raise exception 'CASO 10b: worker_id errado nunca pode cancelar';
  end if;

  if not dental_leads.cancel_reserved_bubble(v_msg, v_job, 'worker-A', v_turn, 'motivo_valido') then
  ...
  if dental_leads.cancel_reserved_bubble(v_msg, v_job, 'worker-A', v_turn, 'motivo_valido_de_novo') then
    raise exception 'CASO 10e: cancelar uma bolha já CANCELLED deveria devolver false, não repetir a ação';
  end if;

  perform dental_leads.close_turn(v_job, 'worker-A', 'STALE_BEFORE_SEND', array[]::uuid[], false, null, 4, 120);
  ...
  insert into dental_leads.jobs (lead_id, status, debounce_started_at, run_after, attempts)
  values (v_lead, 'PENDING', now(), now(), 1) returning id into v_job;
  select job_id, attempts into v_job, v_count from dental_leads.claim_lead_jobs('worker-A', 5, 120);
  -- attempts foi incrementado pelo claim (2 agora)

  if dental_leads.yield_turn(v_job, 'worker-intruso') then
    raise exception 'CASO 11a: worker que não é dono do lease não pode ceder o turno';
  end if;

  if not dental_leads.yield_turn(v_job, 'worker-A') then
    raise exception 'CASO 11b: o dono do lease deveria conseguir ceder';
  end if;

  if (select status from dental_leads.jobs where id = v_job) <> 'PENDING' then
    raise exception 'CASO 11c: job cedido deveria voltar pra PENDING imediatamente';
  end if;
  if (select run_after from dental_leads.jobs where id = v_job) > now() then
    raise exception 'CASO 11d: job cedido deveria estar pronto pra ser pego na hora (run_after <= agora)';
  end if;
  if (select outcome from dental_leads.jobs where id = v_job) <> 'YIELDED' then
    raise exception 'CASO 11e: outcome deveria ser YIELDED, não FAILED (não é erro)';
  end if;
  if (select attempts from dental_leads.jobs where id = v_job) <> v_count then
    raise exception 'CASO 11f: yield não pode gastar tentativa (attempts deveria continuar %, está %)', v_count, (select attempts from dental_leads.jobs where id = v_job);
  end if;

  update dental_leads.jobs set status='DONE' where id = v_job;
  ...
  perform dental_leads.reclaim_expired_jobs();

  if (select status from dental_leads.jobs where id = v_job) <> 'DONE' then
    raise exception 'CASO 12a: job superado deveria estar DONE';
  end if;
  if (select outcome from dental_leads.jobs where id = v_job) <> 'SUPERSEDED' then
    raise exception 'CASO 12b: outcome deveria ser SUPERSEDED, nunca FAILED — não houve erro nenhum';
  end if;
  if (select needs_human from dental_leads.leads where id = v_lead) then
  ...
  if (select conversation_revision from dental_leads.leads where whatsapp_id = '5511999991003@s.whatsapp.net') <> 1 then

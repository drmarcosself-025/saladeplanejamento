...
begin
  ...
  if (select conversation_revision from dental_leads.leads where id = v_lead) <> 1 then
  ...
  select id, run_after, debounce_started_at into v_job, v_run1, v_started
    from dental_leads.jobs where lead_id = v_lead and status = 'PENDING';
  ...
  select count(*) into v_count from dental_leads.jobs where lead_id = v_lead and status = 'PENDING';
  if v_count <> 1 then
    raise exception 'CASO 2: a rajada deveria manter UM único job pendente, encontrei %', v_count;
  end if;

  select run_after into v_run2 from dental_leads.jobs where id = v_job;
  if v_run2 <= v_run1 then
    raise exception 'CASO 2: mensagem nova deveria empurrar a janela de debounce';
  end if;

  if (select debounce_started_at from dental_leads.jobs where id = v_job) <> v_started then
  ...
  -- ---------------------------------------------------------------------
  update dental_leads.jobs set debounce_started_at = now() - interval '29 seconds' where id = v_job;
  ...
  select conversation_revision into v_rev from dental_leads.leads where id = v_lead;
  if v_rev <> 3 then
    raise exception 'CASO 4: reentrega não pode incrementar a revisão (esperado 3, veio %)', v_rev;
  end if;

  select count(*) into v_count from dental_leads.messages where lead_id = v_lead and direction = 'IN';
  if v_count <> 3 then
  ...
  -- ---------------------------------------------------------------------
  update dental_leads.jobs set run_after = now() + interval '10 seconds' where id = v_job;
  if exists (select 1 from dental_leads.claim_lead_jobs('worker-A', 5, 120)) then
    raise exception 'CASO 5: turno com janela de debounce aberta não pode ser claimado';
  end if;

  update dental_leads.jobs set run_after = now() where id = v_job;
  select count(*) into v_count from dental_leads.claim_lead_jobs('worker-A', 5, 120);
  if v_count <> 1 then
    raise exception 'CASO 5: turno maduro deveria ser claimado uma vez, obtive %', v_count;
  end if;

  select turn_id into v_turn from dental_leads.jobs where id = v_job;

  if exists (select 1 from dental_leads.claim_lead_jobs('worker-B', 5, 120)) then
  ...
  -- ---------------------------------------------------------------------
  select count(*) into v_count from dental_leads.fetch_batch(v_lead);
  if v_count <> 3 then
    raise exception 'CASO 6: o batch deveria conter as 3 mensagens da rajada, contei %', v_count;
  end if;

  if (select array_agg(id order by received_at, id) from dental_leads.fetch_batch(v_lead))
     <> array[v_msg1, v_msg2, v_msg3] then
    raise exception 'CASO 6: o batch saiu fora da ordem cronológica';
  end if;

  if (select distinct conversation_revision from dental_leads.fetch_batch(v_lead)) <> 3 then
  ...
  -- ---------------------------------------------------------------------
  v_result := dental_leads.assert_turn_valid(p_job_id=>v_job, p_worker_id=>'worker-A', p_turn_id=>v_turn, p_lead_id=>v_lead, p_batch_ids=>array[v_msg1, v_msg2, v_msg3], p_input_revision=>3, p_purpose=>'AI_REPLY');
  if not (v_result->>'valid')::boolean then
    raise exception 'CASO 7a: turno íntegro deveria ser válido (%)', v_result->>'reason';
  end if;

  v_result := dental_leads.assert_turn_valid(p_job_id=>v_job, p_worker_id=>'worker-B', p_turn_id=>v_turn, p_lead_id=>v_lead, p_batch_ids=>array[v_msg1, v_msg2, v_msg3], p_input_revision=>3, p_purpose=>'AI_REPLY');
  ...
  v_result := dental_leads.assert_turn_valid(p_job_id=>v_job, p_worker_id=>'worker-A', p_turn_id=>gen_random_uuid(), p_lead_id=>v_lead, p_batch_ids=>array[v_msg1, v_msg2, v_msg3], p_input_revision=>3, p_purpose=>'AI_REPLY');
  ...
  v_result := dental_leads.assert_turn_valid(p_job_id=>v_job, p_worker_id=>'worker-A', p_turn_id=>v_turn, p_lead_id=>v_lead, p_batch_ids=>array[v_msg1, v_msg2, v_msg3], p_input_revision=>2, p_purpose=>'AI_REPLY');
  ...
  v_result := dental_leads.assert_turn_valid(p_job_id=>v_job, p_worker_id=>'worker-A', p_turn_id=>v_turn, p_lead_id=>v_lead, p_batch_ids=>array[v_msg1, v_msg2], p_input_revision=>3, p_purpose=>'AI_REPLY');
  ...
  update dental_leads.leads set automation_status = 'HUMAN_REQUIRED', needs_human = true where id = v_lead;
  v_result := dental_leads.assert_turn_valid(p_job_id=>v_job, p_worker_id=>'worker-A', p_turn_id=>v_turn, p_lead_id=>v_lead, p_batch_ids=>array[v_msg1, v_msg2, v_msg3], p_input_revision=>3, p_purpose=>'AI_REPLY');
  if (v_result->>'valid')::boolean then
    raise exception 'CASO 7f: resposta da IA não pode sair com lead aguardando humano';
  end if;

  v_result := dental_leads.assert_turn_valid(p_job_id=>v_job, p_worker_id=>'worker-A', p_turn_id=>v_turn, p_lead_id=>v_lead, p_batch_ids=>array[v_msg1, v_msg2, v_msg3], p_input_revision=>3, p_purpose=>'HANDOFF');
  if not (v_result->>'valid')::boolean then
    raise exception 'CASO 7g: a frase neutra PODE sair em HUMAN_REQUIRED (%)', v_result->>'reason';
  end if;

  update dental_leads.leads set automation_status = 'ACTIVE', needs_human = false where id = v_lead;
  ...
  perform dental_leads.close_turn(v_job, 'worker-A', 'COMPLETED', array[v_msg1, v_msg2, v_msg3], true, null, 4, 120);
  ...
  select id into v_job2 from dental_leads.jobs where lead_id = v_lead and status = 'PENDING';
  ...
  if (select outcome from dental_leads.jobs where id = v_job) <> 'SUPERSEDED' then
    raise exception 'CASO 9b2: outcome deveria ser SUPERSEDED (não é erro), veio %', (select outcome from dental_leads.jobs where id = v_job);
  end if;
  ...
  if (select status from dental_leads.jobs where id = v_job2) <> 'PENDING' then
  ...
  update dental_leads.jobs set run_after = now() where id = v_job2;
  select count(*) into v_count from dental_leads.claim_lead_jobs('worker-C', 5, 120);
  if v_count <> 1 then
    raise exception 'CASO 9f: Job B deveria ser claimável normalmente após A ser encerrado';
  end if;

  select turn_id into v_turn from dental_leads.jobs where id = v_job2;
  perform dental_leads.close_turn(v_job2, 'worker-C', 'NO_REPLY', array[]::uuid[], false, null, 4, 120);
  ...
  select id into v_job from dental_leads.jobs where lead_id = v_lead and status = 'PENDING';
  update dental_leads.jobs set run_after = now() where id = v_job;
  perform dental_leads.claim_lead_jobs('worker-A', 5, 120);

  perform dental_leads.close_turn(v_job, 'worker-A', 'STALE_BEFORE_SEND', array[v_msg1], false, null, 4, 120);
  if (select consumed_at from dental_leads.messages where id = v_msg1) is not null then
    raise exception 'CASO 10: turno abortado antes do envio NÃO pode marcar a mensagem como consumida';
  end if;
  if not exists (select 1 from dental_leads.fetch_batch(v_lead) where id = v_msg1) then
  ...
  perform dental_leads.close_turn(v_job, 'worker-A', 'COMPLETED', array[v_msg1], true, null, 4, 120);
  if (select consumed_at from dental_leads.messages where id = v_msg1) is null then
    raise exception 'CASO 11a: turno concluído deveria marcar consumed_at';
  end if;
  if (select consumed_by_turn_id from dental_leads.messages where id = v_msg1) <> v_turn then
    raise exception 'CASO 11b: consumed_by_turn_id deveria apontar para o turno que consumiu a mensagem';
  end if;
  if exists (select 1 from dental_leads.fetch_batch(v_lead) where id = v_msg1) then
  ...
  perform dental_leads.close_turn(v_job, 'worker-A', 'FAILED', array[]::uuid[], false, 'erro de rede', 4, 120);
  ...
  if (select next_attempt_at from dental_leads.jobs where id = v_job) <= now() then
    raise exception 'CASO 12a: o retry deveria respeitar backoff';
  end if;

  update dental_leads.jobs set attempts = 4, status = 'RUNNING', locked_by = 'worker-A',
         lease_expires_at = now() + interval '120 seconds'
   where id = v_job;
  perform dental_leads.close_turn(v_job, 'worker-A', 'FAILED', array[]::uuid[], false, 'erro persistente', 4, 120);

  if (select status from dental_leads.jobs where id = v_job) <> 'FAILED' then
  ...
  if dental_leads.reclaim_expired_jobs() <> 1 then
  ...
  perform dental_leads.claim_lead_jobs('worker-A', 5, 120);
  if not dental_leads.renew_lease(v_job, 'worker-A', 120) then
    raise exception 'CASO 13b: o dono do lease deveria conseguir renovar';
  end if;
  if dental_leads.renew_lease(v_job, 'worker-B', 120) then
  ...
  select count(*) into v_count from dental_leads.claim_lead_jobs('worker-D', 5, 120);
  if v_count <> 2 then
  ...
  if exists (
    select 1 from dental_leads.jobs j
      join dental_leads.leads l on l.id = j.lead_id
  ...
  if (select send_status from dental_leads.messages where provider_message_id = 'MSG-ECO') <> 'SENT' then
